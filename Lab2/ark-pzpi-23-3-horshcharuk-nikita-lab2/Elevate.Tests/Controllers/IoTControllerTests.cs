...
public class IoTControllerTests : ControllerTestBase
{
    ...
    [Fact]
    public async Task Scan_WithValidDeviceAndUser_ReturnsOk()
    {
        // Arrange
        ...
        context.AddRange(team, level, user, membership, actionType, device);
        ...
        var result = await response.Content.ReadFromJsonAsync<DeviceScanResponseDto>();
    ...
    [Fact]
    public async Task Scan_WithUnauthorizedUser_ReturnsUnauthorized()
    {
        // Arrange
        await using var scope = Factory.Services.CreateAsyncScope(); var context = scope.ServiceProvider.GetRequiredService<Elevate.Data.ElevateDbContext>();
    ...
    [Fact]
    public async Task Scan_WithoutActionCode_ReturnsOk()
    {
        // Arrange
        await using var scope = Factory.Services.CreateAsyncScope(); var context = scope.ServiceProvider.GetRequiredService<Elevate.Data.ElevateDbContext>();
        ...
        context.AddRange(team, user, membership, device);
        await context.SaveChangesAsync();
