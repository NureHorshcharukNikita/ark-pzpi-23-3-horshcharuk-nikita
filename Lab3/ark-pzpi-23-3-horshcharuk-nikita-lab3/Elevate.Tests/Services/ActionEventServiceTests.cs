...
{
    [Fact]
    public async Task CreateActionEventAsync_IncrementsPoints_AssignsLevelAndBadge()
    {
        // Arrange
        ...
        var validator = new ActionEventValidator(context);
        var sut = new ActionEventService(context, gamificationService, validator);
        ...
        // Act
        var result = await sut.CreateActionEventAsync(dto, user.UserID, CancellationToken.None);
    ...
    [Fact]
    public async Task GetActionEventsAsync_FiltersByUserAndTeam()
    {
        // Arrange
        await using var context = TestContextFactory.CreateContext();
        ...
        // Act
        var result = await sut.GetActionEventsAsync(user.UserID, backendTeam.TeamID, CancellationToken.None);
